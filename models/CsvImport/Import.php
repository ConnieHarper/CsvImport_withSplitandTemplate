...
public function logit($data){
	$handle = fopen( '/var/www/html/dev/cfmi/omeka/trunk/archive/harvestLog.txt','a');
	fwrite ($handle, $data);	
	fclose($handle);
}	
	/**
   ...
   */
            ...
            foreach($columnMaps as $columnMap) {
                ...
                foreach($elementIds as $elementId) {
                    ...
                    $elementInfo = array('element_name' => $element->name, 
                                         ...
                                         'element_text_is_split' => $columnMap->getDataIsSplit(),
	...
	private function addItemFromRow(&$row, &$itemMetadata, &$colNumToElementInfosMap, &$colNumMapsToTag, &$colNumMapsToFile) 
	{
	    ...
	    for($colIndex = 0; $colIndex < count($row); $colIndex++) {
	        ...
	        if ( $colNumToElementInfosMap[$colIndex] !== null) {
    	        ...
    	        foreach($elementInfos as &$elementInfo) {
        	        ...
        	        $elementTextIsSplit = (boolean) $elementInfo['element_text_is_split'];
			...
			// Will split the columnValue internally to make them into seperate entries
			if ($elementTextIsSplit){
			        // process the splits
		               	$rawSplits = explode(';', $columnValue);
               		 	foreach($rawSplits as &$rawSplit) {
		               		$split = trim($rawSplit);
        	        		// add the element text from the column value
        	        		$itemElementText = array('text' => $split, 'html' => $elementTextIsHtml);
	        ...
	        if ($colNumMapsToFile[$colIndex]) {
                ...
                $rawSplits = explode(';', $urlForFile);
                foreach($rawSplits as &$rawSplit) {
                if (!in_array($rawSplit, $urlsForFiles) && ($rawSplit != "")) {
                    $urlsForFiles[] = CSV_IMPORT_CSV_FILES_DIRECTORY.DIRECTORY_SEPARATOR.$rawSplit;
                }       
		} 
            }        
	    }
	    ...
	    if (count($urlsForFiles) > 0) {
	        $fileMetadata = array('file_transfer_type' => 'Filesystem', 'files' => $urlsForFiles);
	    } else {
	        $fileMetadata = array();
	    }
...
//		$db = get_db();
 //               $sql = "call omeka_db.spInsertLocation(".$item['id'].")";
  //              $db->query($sql);
	...
	private function insert_resource($itemMetaData = array(), $itemElementTexts = array()){
                    /**$t1 = $db->getTable('cfmi_primary_content_type');
		    $elementText = $itemElementTexts
                    $primary = $t1->findBySql('where `name` = ?',$elementText, true);
**/
//example        	    $sql = $iit->getSelect()->where('`import_id` = ?')->limit($itemLimitPerQuery);
                   /** 
		    $t2 = $db->getTable('cfmi_secondary_content_type');
                    $secondary = $t1->findBySql('where `name` = ?',$elementText, true);
$this->logit("\nsecondary".$secondary);
                    
		    $t3 = $db->getTable('cfmi_tertiary_content_type');
                    $third = $t1->findBySql('where `name` = ?',$elementText, true);
$this->logit("\nThird".$third);***/
