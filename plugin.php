 ...
 * Modified by UCF  RICHES, 2012 to add a "Split" column.  This allows one column in the CSV file to
 * be split into multiple entries for that element. 
 * The character that it splits on  * is a semi-colon ';'.  Only splits if the split check box is checked.
 **/
...
define('CSV_IMPORT_COLUMN_MAP_SPLIT_CHECKBOX_PREFIX', 'csv_import_column_map_split_');
...
{   
    ...
    for($i = 0; $i < count($colNames); $i++) {
        ...
        $ht .= '<td>'.csv_import_checkbox(CSV_IMPORT_COLUMN_MAP_SPLIT_CHECKBOX_PREFIX . $i).'</td>';
        $ht .= '</tr>';
    }
	$ht .= '</tbody>';
	$ht .= '</table>';
